...
const MatterLabelEdit = (props) => {
  const { CloseMatterLabelModal, setCaller,setLabelId, labelId, caller} = props.callbackFunction
  ...
  useEffect(() => {
    ...
    if(caller == "edit"){
      SelectMatterLabel(labelId)
    }
    
  }, [caller]);

  const SelectMatterLabel = useCallback(async(id: number) => {
  ...
  const saveMatterLabel = useCallback(async() => {
    try {
          ...
          <div style={{display:"flex"}}>
            ...
            <div className='checkBoxDiv'>
              <input
                ...
                className='checkbox'
                name="select"
                checked={flg_Simplificada}
                onChange={() => setFlg_Simplificada(!flg_Simplificada)}
              />
            </div>

            <MdHelp className='infoMessage' title="O layout simplificado exibe pasta, processo e partes na etiqueta. O layout regular exibe pasta, partes, processo, vara, forum, distribuição e advogado." />
            
          </div>

          <p style={{marginTop:"2%"}}>Informe as dimensões da etiqueta em centímetros:</p>
...
export default MatterLabelEdit;
