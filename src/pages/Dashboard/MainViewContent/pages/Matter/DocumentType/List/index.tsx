...
export interface IDocumentTypeData{
  ...
  const [documentTypeList, setDocumentTypeList] = useState<IDocumentTypeData[]>([]);
  ...
  const columns = [
    { name: 'documentTypeDescription', title: ' '},
    { name: 'edit',  title: ' '},
    { name: 'remove',title: ' '}
  ];

  const [tableColumnExtensions] = useState([
    { columnName: 'documentTypeDescription', width: '75%' },
    { columnName: 'btnEditar', width: '5%' },
    { columnName: 'btnRemover',width: '5%' },
  ]);

  const [tableColumnExtensionsMobile] = useState([
    { columnName: 'documentTypeDescription', width: '40%' },
  ...
  useEffect(() => {
    if (!modalActive && caller == 'documentTypeModal'){
        LoadDocumentType('initialize')
    }
  }, [modalActive])

  
  useEffect(() => {
    if (caller == 'documentTypeModal' && modalActive){
      setShowModal(true)      
    }
  },[caller, modalActive])
  ...
  // METHODS
  const LoadDocumentType = useCallback(async(state = '') => {
    try {  
      ...
      const response = await api.get<IDocumentTypeData[]>('/TipoDocumento/ListarPorFiltro', { 
      ...
      if (!isPagination || state === 'initialize'){
        ...
        response.data.map((item)=> documentTypeList.push(item))
  ...
  const handleClick = (props: any) => {
    ...
    if (props.column.name === 'edit'){
      handleEdit(props.row.documentTypeId)
    }

    if (props.column.name === 'remove'){
      deleteDocumentType(props.row.documentTypeId)
    }
  }
  ...
  const deleteDocumentType = async(id: number) => {
    ...
    try {
      ...
      addToast({
        ...
        description: "O tipo de documento foi excluído no sistema."
      })
      ...
      addToast({
        type: "error",
        title: "Falha ao excluir tipo de documento.",
  ...
  function handleScroll(e: UIEvent<HTMLDivElement>){
    ...
    if (isEndPage || documentTypeList.length == 0) return;
            ...
            <div style={{float:'right', marginRight:'170px'}}>
              <div style={{float:'left'}}>
                <button 
                  ...
                  title="Clique para incluir um tipo de documento"
                  type="submit"
