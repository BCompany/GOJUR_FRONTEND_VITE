...
const CourtList = () => {
   ...
   // USE EFECT
    ...
    useEffect(() => {
      if (!modalActive && caller == 'courtModal'){
          LoadCourt('initialize')
      }
    }, [modalActive])
  
    
    useEffect(() => {
      if (caller == 'courtModal' && modalActive){
        setShowModal(true)      
      }
    },[caller, modalActive])
   ...
   // METHODS
    const LoadCourt = useCallback(async(state = '') => {
      try {
        ...
        const response = await api.get<ICourtData[]>('/Forum/Listar', { 
        ...
        if (!isPagination || state === 'initialize'){
          ...
          response.data.map((item)=> courtList.push(item))
    ...
    if (column.name === 'forumName') {
      return (
        
        <Table.Cell {...props}>
          <div style={{float:'left',width:'60%'}}>
            <span style={{color:'black', fontSize:'0.7rem', fontFamily:'Arial'}}>{props.row.forumName}</span>
            <br />
            <br />
            <span style={{color:'black', fontSize:'0.6rem', fontFamily:'Arial', fontWeight:600, }}>Classe:&nbsp;</span>
            <span style={{color:'black', fontSize:'0.6rem', fontFamily:'Arial'}}>{props.row.forumClass}</span> 
          </div>
          
          <div style={{float:'left',width:'40%'}}>
            <span style={{color:'black', fontSize:'0.75rem', fontFamily:'Arial'}}>{props.row.federalUnitName}</span>
          </div>
    ...
    if (column.name === 'forumName') {
      return (
        ...
        <Table.Cell {...props} style={{paddingLeft:'5px'}}>
          <div style={{float:'left',width:'60%'}}>
            <span style={{color:'black', fontSize:'0.68rem', fontFamily:'Arial'}}>{props.row.forumName}</span>
            <br />
            <span style={{color:'black', fontSize:'0.6rem', fontFamily:'Arial', fontWeight:600, }}>Classe:&nbsp;</span>
            <span style={{color:'black', fontSize:'0.6rem', fontFamily:'Arial'}}>{props.row.forumClass}</span>
            <br />
            <span style={{color:'black', fontSize:'0.6rem', fontFamily:'Arial', fontWeight:600, }}>Estado:&nbsp;</span> 
            <span style={{color:'black', fontSize:'0.6rem', fontFamily:'Arial'}}>{props.row.federalUnitName}</span>
          </div>
    ...
    const handleClick = (props: any) => {
      ...
      if (props.column.name === 'edit'){
        handleEdit(props.row.forumId)
      }
  
      if (props.column.name === 'remove'){
        deleteCourtDept(props.row.forumId)
      }
    }
    // EDIT
    ...
    const deleteCourtDept = async(id: number) => {
      ...
      try {
        ...
        setIsDeleting(true)
        await api.delete('/Forum/Deletar', {
          params:{
          id,
          token
          }
        ...
        addToast({
          ...
          description: "o forum foi excluído no sistema."
        })
    ...
    function handleScroll(e: UIEvent<HTMLDivElement>){
      ...
      if (isEndPage || courtList.length == 0) return;
          ...
          <div style={{float:'right', marginRight:'170px'}}>
            <div style={{float:'left'}}>
              <button 
                ...
                title="Clique para incluir um fórum"
                type="submit"
