...
import MatterProbabilityEdit from '../Modal';
...
export interface IMatterProbabilityData{
  ...
  const [matterProbabilityList, setMatterProbabilityList] = useState<IMatterProbabilityData[]>([]);
  ...
  useEffect(() => {
    if (!modalActive && caller == 'matterProbabilityModal'){
        LoadMatterProbability('initialize')
    }
  }, [modalActive])

  
  useEffect(() => {
    if (caller == 'matterProbabilityModal' && modalActive){
      setShowModal(true)      
    }
  },[caller, modalActive])

  useEffect(() => {
    setMatterProbabilityList([])
  ...
  // METHODS
  const LoadMatterProbability = useCallback(async(state = '') => {
    ...
    try {
        const response = await api.get<IMatterProbabilityData[]>('/ProbabilidadeProcesso/Listar', { 
          params:{
          page,
          rows:20,
      ...
      if (!isPagination || state === 'initialize'){
        setIsEndPage(false)
        setMatterProbabilityList(response.data)    
      }
      else{
        response.data.map((item)=> matterProbabilityList.push(item))
        setMatterProbabilityList(matterProbabilityList)    
      }
  ...
  const handleClick = (props: any) => {
    ...
    if (props.column.name === 'remove'){
      deleteMatterProbability(props.row.id)
    }
  }
  ...
  // DELETE
  const deleteMatterProbability = async(id: number) => {
   
    try {
      const token = localStorage.getItem('@GoJur:token');
      
      setIsDeleting(true)
      await api.delete('/ProbabilidadeProcesso/Deletar', {
        params:{
        id,
        token
        }
      ...
      addToast({
        type: "success",
        title: "Probabilidade do processo excluída",
        description: "A probabilidade do processo foi excluída no sistema."
      })
      ...
      addToast({
        type: "error",
        title: "Falha ao excluir probabilidade do processo.",
  ...
  function handleScroll(e: UIEvent<HTMLDivElement>){
    ...
    if (isEndPage || matterProbabilityList.length == 0) return;
            ...
            <div style={{float:'right', marginRight:'170px'}}>
              <div style={{float:'left'}}>
                <button 
                  ...
                  title="Clique para incluir uma probabilidade do processo"
                  type="submit"
...
export default MatterProbabilityList