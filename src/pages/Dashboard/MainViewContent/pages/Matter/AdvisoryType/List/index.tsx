...
import AdvisoryTypeEdit from '../Modal';
...
const AdvisoryTypeList = () => {
  ...
  const [advisoryTypeList, setAdvisoryTypeList] = useState<IAdvisoryTypeData[]>([]);
  ...
  useEffect(() => {
    if (!modalActive && caller == 'advisoryTypeModal'){
        LoadAdvisoryType('initialize')
    }
  }, [modalActive])
  ...
  // METHODS
  const LoadAdvisoryType = useCallback(async(state = '') => {
    ...
    try{
      ...
      const response = await api.get<IAdvisoryTypeData[]>('/TipoConsultivo/Listar', { 
        params:{
          page,
          rows:20,
      ...
      if (!isPagination || state === 'initialize'){
        setIsEndPage(false)
        setAdvisoryTypeList(response.data)    
      }
      else{
        response.data.map((item)=> advisoryTypeList.push(item))
        setAdvisoryTypeList(advisoryTypeList)    
      }
  ...
  const handleClick = (props: any) => {
    ...
    if (props.column.name === 'remove'){
      deleteAdvisoryType(props.row.id)
    }
  }
  ...
  // DELETE
  const deleteAdvisoryType = async(id: number) => {
   
    try {
      const token = localStorage.getItem('@GoJur:token');

      setIsDeleting(true)
      await api.delete('/TipoConsultivo/Delete', {
        params:{
        id,
        token
        }
      ...
      addToast({
        type: "success",
        title: "Assunto consultivo excluído",
        description: "O assunto consultivo foi excluído no sistema."
      })
      ...
      addToast({
        type: "error",
        title: "Falha ao excluir assunto consultivo.",
  ...
  function handleScroll(e: UIEvent<HTMLDivElement>){
    ...
    if (isEndPage || advisoryTypeList.length == 0) return;
            ...
            <div style={{float:'right', marginRight:'170px'}}>
              <div style={{float:'left'}}>
                <button 
                  ...
                  title="Clique para incluir um assunto"
                  type="submit"
...
export default AdvisoryTypeList;