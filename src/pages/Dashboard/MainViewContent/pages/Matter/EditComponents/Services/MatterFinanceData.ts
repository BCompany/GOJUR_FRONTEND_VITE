...
export const ListMatterFinance = async (matterId, accountId, page, rows) => {
  ...
  const response = await api.get<IMatterFinance[]>('FinanceiroProcesso/Listar', {
    params: {matterId, page, rows, accountId, filterClause: '', token}
  })
...
export const DeleteMatterFinance = async (id: number, deleteAll: boolean, validateFinancialIntegration: boolean) => {
  try{
    const token = localStorage.getItem('@GoJur:token')

    const response = await api.delete<IMatterFinance>('Financeiro/Deletar', {
      params: {id, deleteAll, validateFinancialIntegration, token}
    });
    ...
    if (err.response.data.typeError.warning == "awareness")
      return err.response.data.typeError.warning;
  }
}
...
export const ListAccount = async (term: string) => {
  const token = localStorage.getItem('@GoJur:token')
  const response = await api.post<IDefaultsProps[]>(`/ContasBancarias/Listar`,{token, term});
  return response;
}
