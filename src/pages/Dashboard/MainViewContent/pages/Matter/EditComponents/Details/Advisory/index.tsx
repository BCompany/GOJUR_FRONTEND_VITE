...
import { CalculateMatterValue, ComboSelectType, DeleteMatterEvents, DeleteMatter, GetMatterValuesJSON, ListAdvisoryType, ListMatterStatus, SaveMatter, SaveMatterAttach, SaveMatterMarkers, SelectFolderCode, SelectMatter } from '../../Services/MatterData';
...
const Matter = (props) => {
  ...
  const [matterAdvisoryTypeId, setMatterAdvisoryTypeId] = useState<number>()
  ...
  const [matterAdvisoryTypeList, setMatterAdvisoryTypeList] = useState<ISelectData[]>([])
  const [matterStatusList, setMatterStatusList] = useState<ISelectData[]>([])
  const accessCode = localStorage.getItem('@GoJur:accessCode')
  const showButtonDelete = (accessCode??"").includes('MATCDE') || (accessCode??"") == 'adm'
  ...
  const EditMatter = async () => {
    ...
    setMatterAdvisoryTypeId(matter.typeAdvisorId)
  ...
  const LoadComboDataSelected = (matter: IMatterData) => {
    ...
    // type advosory
    if (matter.typeAdvisorId > 0) {
      matterAdvisoryTypeList.push({
        id:matter.typeAdvisorId.toString(),
        label:matter.typeAdivisor
      })
      setMatterAdvisoryTypeList(matterAdvisoryTypeList)
    }
  ...
  const LoadComboData = async (matter: IMatterData = {} as IMatterData) => {
    ...
    if (!matter.typeAdvisorId) {
      setMatterAdvisoryTypeList(await ListAdvisoryType(0, 50,''))
    }
...
// Effect to update risk value when was clicked on apply to risk value on matter value cause
  ...
  const handleResponsibleListCallback = (userList: ISelectData[]) => {
  ...
  const handleReactSelectChange = async (identity: ComboSelectType, select) => {
    ...
    if (select){
      ...
      if (identity == ComboSelectType.advisororyType){
        setMatterAdvisoryTypeId(select.id)
      }
  ...
  const RefreshComboSelect = async (identity: ComboSelectType, filterTerm: string) => {
    ...
    if (identity === ComboSelectType.advisororyType){
      setMatterAdvisoryTypeId(0)
      const response = await ListAdvisoryType(0, 50, filterTerm);
      setMatterAdvisoryTypeList(response)
    }
  ...
  const handleExecuteCalculation = async (data: IMatterCalculationData, calculationId: number | undefined) => {
      ...
      const jsonCalculator = JSON.stringify(data);

      const resultCalculation = await CalculateMatterValue(jsonCalculator)
  ...
  const handleSaveMatter = useCallback(async() => {
    ...
    try
    {
      ...
      const data = {
        ...
        cod_TipoConsultivo:matterAdvisoryTypeId,
      ...
      if (matterIdSaved && newMatter){
        ...
        history.push(`/matter/edit/advisory/${  matterIdSaved}`)
        EditMatter()
      }
  ...
  },[matterFolder,responsibleList, matterNumberCNJ, matterNumber, matterAdvisoryTypeId, matterSecurity, matterTitle, matterOrder, matterStatusId,matterDateRelease, natterDateFinalization,matterDateEntrance, matterUserCourt, matterUserCourtPsw, matterObjectDescription, matterSequence, matterAmazonS3Type, matterDataValues,markerList,matterFlagRobot, matterLoadId, matterFollowWebDate])
  ...
  const handleAttachMatter = useCallback(() => {
    ...
    if (matterId == 0){
      addToast({
        type: 'info',
        title: 'Operação NÃO realizada',
        description: 'É necessário salvar o processo antes apensar um processo'
      });
      return
    }
        ...
        <div className='content'>
          ...
          <section>
            ...
            <label>
              ...
              <Select
                isSearchable
                isClearable
                isLoading={currentListData === ComboSelectType.advisororyType && isLoadingCombo}
                options={matterAdvisoryTypeList}
                value={matterAdvisoryTypeList.find(item => Number(item.id) === matterAdvisoryTypeId)}
                onInputChange={(term) => handleReactInputText(ComboSelectType.advisororyType, term)}
                onChange={(item) => handleReactSelectChange(ComboSelectType.advisororyType, item)}
          ...
          <section>
            ...
            <label style={{flex:'25%'}}>
              ...
              <input
                ...
                value={matterTitle}
              />
              <MdHelp className='infoMessage' title="Informe uma especificação de ação (Apelido)" />
            </label>
          ...
          <div className='valores'>
            ...
            <div>
              ...
              <label>Valor</label>
              <label>Data</label>
              <label style={{marginLeft:'45px'}}>Valor Atual</label>
