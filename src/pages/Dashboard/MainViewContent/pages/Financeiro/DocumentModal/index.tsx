...
const FinancialDocumentModal = (props) => {

  const {movementId, CloseDocumentModal} = props.callbackFunction
  ...
  const VisualizeDocument = useCallback(async () => {
    ...
    const response = await api.post('/DocumentosModelo/VerDocumentoFinanceiro',
      {
    ...
    localStorage.setItem('@GoJur:documentLocation', 'financial');

    CloseModal()
    history.push(`/documentmodel/visualize/${documentModelId? Number(documentModelId): 0}`)

  }, [documentModelId, movementId]);
  ...
  const GenerateDocument = useCallback(async () => {
    ...
    const response = await api.post('/DocumentosModelo/GerarReciboFinanceiro',
      {
        id: Number(movementId),
        documentModelId: Number(documentModelId),
        token,
