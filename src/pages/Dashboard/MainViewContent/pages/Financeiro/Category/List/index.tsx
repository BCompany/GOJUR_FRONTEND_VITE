...
import CategoryEdit from '../Modal';
...
const CategoryList = () => {
  ...
  useEffect(() => {
    if (!modalActive && caller == 'categoryModal'){
    LoadCategory('initialize')
    }
  }, [modalActive])

  useEffect(() => {
    if (caller == 'categoryModal' && modalActive){
    setShowModal(true)      
}
  },[caller, modalActive])
  ...
  const LoadCategory = useCallback(async(state = '') => {
    try {
      ...
      const response = await api.get<ICategoryData[]>('/Categoria/Listar', { 
          params:{
          page,
          rows:20,
      ...
      if (!isPagination || state === 'initialize'){
        ...
        response.data.map((item)=> categoryList.push(item))
  ...
  const handleClick = (props: any) => {
    ...
    if (props.column.name === 'edit'){
      handleEdit(props.row.categoryId)
    }

    if (props.column.name === 'remove'){
      deleteCategory(props.row.categoryId)
    }
  }
  ...
  const deleteCategory = async(id: number) => {
    ...
    try {
      ...
      await api.delete('/Categoria/Deletar', {
        params:{
        id,
        token
        }
      ...
      addToast({
        ...
        description: "A categoria foi excluída no sistema."
      })
  ...
  function handleScroll(e: UIEvent<HTMLDivElement>){
    ...
    if (isEndPage || categoryList.length == 0) return;
            ...
            <div style={{float:'right', marginRight:'170px'}}>
              <div style={{float:'left'}}>
                <button 
                  ...
                  title="Clique para incluir uma categoria"
                  type="submit"
